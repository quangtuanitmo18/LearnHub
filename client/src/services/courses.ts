...
export class CoursesService {
  ...
  // Get public courses
  static async getPublicCourses(params?: CoursesListParams): Promise<PublicCoursesListResponse> {
    try {
  ...
  static async createCourse(courseData: CreateCourseRequest): Promise<ICourse> {
    return ApiService.post<ICourse, CreateCourseRequest>(ENDPOINTS.COURSES, courseData);
  }

  // Update course
  static async updateCourse(courseData: UpdateCourseRequest): Promise<ICourse> {
    const { id, ...updateData } = courseData;
    return ApiService.put<ICourse, Partial<CreateCourseRequest>>(ENDPOINTS.COURSE(id), updateData);
  }

  // Patch course
  static async patchCourse(id: string, courseData: Partial<CreateCourseRequest>): Promise<ICourse> {
  ...
  static async bulkDeleteCourses(courseIds: string[]): Promise<void> {
    return ApiService.delete<void, { courseIds: string[] }>(`${ENDPOINTS.COURSES}/bulk-delete`, {
      courseIds,
    });
  }

  // Update course status
  static async updateCourseStatus(id: string, status: string): Promise<ICourse> {
  ...
  // Update free status
  static async updateCourseFreeStatus(id: string, isFree: boolean): Promise<ICourse> {
  ...
  static async getRelatedCourses(courseId: string): Promise<IPublicCourse[]> {
    try {
      return await ApiService.get<IPublicCourse[]>(`/courses/${courseId}/related`);
    } catch {
      return [];
    }
  }
