...
export class ReviewsService {
	...
	static async getCourseReviewStats(
		courseId: string
	): Promise<CourseReviewStats> {
		return ApiService.get<CourseReviewStats>(
			ENDPOINTS.COURSE_REVIEW_STATS(courseId)
		);
	}
	...
	static async updateReviewStatus(
		...
		return ApiService.put<IReview, { status: string }>(
			`${ENDPOINTS.REVIEWS}/${reviewId}/status`,
			{ status }
		);
	}
	...
	static async bulkDeleteAdminReviews(reviewIds: string[]): Promise<void> {
		return ApiService.delete<void, { reviewIds: string[] }>(
			`${ENDPOINTS.REVIEWS}/bulk`,
			{ reviewIds }
		);
	}
}
