...
const CourseOutlinePage = () => {
  ...
  const handleLessonReorder = (
    chapterId: string,
    reorderedLessons: ILesson[]
  ) => {
    // Optimistic Update: Update lessons cache immediately
    queryClient.setQueryData(
      lessonsKeys.chapterLessons(chapterId),
      reorderedLessons
    );
  ...
  const handleToggleLessonPublish = (lessonId: string) => {
    toggleLessonPublishMutation.mutate(lessonId, {
      onSuccess: (updatedLesson) => {
        toast.success(
          `Lesson ${
            updatedLesson.published ? "published" : "unpublished"
          } successfully`
        );
      },
      onError: () => {
  ...
  const { totalLessons, publishedLessons, totalDuration } = (
    chaptersToRender || []
  ).reduce(
    (acc, chapter) => ({
      totalLessons: acc.totalLessons + (chapter.totalLessons || 0),
      publishedLessons:
        acc.publishedLessons + (chapter.totalPublishedLessons || 0),
                  ...
                  <div className="bg-white rounded-lg shadow-lg border border-blue-500 p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">
                        {chaptersToRender?.find((c) => c.id === activeId)
                          ?.title || "Item"}
                      </span>
                    </div>
                  </div>
