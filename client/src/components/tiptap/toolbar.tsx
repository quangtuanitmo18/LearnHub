  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className="flex h-7 min-w-9 shrink-0 items-center justify-center overflow-hidden rounded-sm px-2 text-sm hover:bg-neutral-200/80"
        >
      ...
      <DropdownMenuContent className="p-0">
        {headings.map(({ label, icon: Icon, onClick, isActive }) => (
          <button
            ...
            onClick={onClick}
            className={cn(
              'flex w-full items-center gap-x-2 px-2 py-1 hover:bg-neutral-200/80',
...
const HighlightColorButton = () => {
  const { editor } = useEditorStore();
  const [color, setColor] = useState(editor?.getAttributes('highlight').color || '#ffff00');
...
const Toolbar = ({}) => {
  ...
  return (
    <div className="flex min-h-10 items-center gap-x-0.5 overflow-x-auto rounded-md px-2.5 py-0.5">
      {section[0].map((item) => (
