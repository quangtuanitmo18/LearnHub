...
export function MembershipBadge({
  ...
  return (
    ...
    >
      ...
      {/* Label */}
      {showLabel && (
        <span className={cn('font-semibold', config.textColor, sizes.text)}>{config.label}</span>
      )}

      {/* Sparkle for premium tiers */}
      {(plan === MembershipPlan.GOLD || plan === MembershipPlan.DIAMOND) && animated && (
        <Sparkles
          className={cn(
...
export function MembershipIcon({ plan, size = 'md', className }: MembershipIconProps) {
  const config = planConfig[plan];
  const Icon = config.icon;
