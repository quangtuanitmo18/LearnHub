...
function ProtectedRouteInner({
  ...
  const userRoles = useMemo(() => user?.roles?.map((role) => role.name) || [], [user?.roles]);

  // Helper function to get the highest priority role for redirects
  const getPrimaryRole = useCallback(() => {
    if (userRoles.includes(SYSTEM_ROLE_NAMES.SUPER_ADMIN)) return SYSTEM_ROLE_NAMES.SUPER_ADMIN;
    if (userRoles.includes(SYSTEM_ROLE_NAMES.ADMIN)) return SYSTEM_ROLE_NAMES.ADMIN;
  ...
  const hasResourceAccess = !resource || !action || canPerformAction(resource, action);

  if (requireAuth && isAuthenticated && (!hasRoleAccess || !hasResourceAccess)) {
...
export function AuthOnlyRoute({ children, ...props }: Omit<ProtectedRouteProps, 'requireAuth'>) {
  return (
