...
const courseFormSchema = yup.object({
  ...
  price: yup
    ...
    .default(0)
    .test('required-when-not-free', 'Price is required for paid courses', function (value) {
  ...
  oldPrice: yup
    ...
    .transform((value) => (isNaN(value) ? 0 : value))
    .default(0)
    .test('greater-than-price', 'Old price should be greater than current price', function (value) {
  ...
  level: yup.mixed<CourseLevel>().oneOf(Object.values(CourseLevel)).required('Level is required'),
});
